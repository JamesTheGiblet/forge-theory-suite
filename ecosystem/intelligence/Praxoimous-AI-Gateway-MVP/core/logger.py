...
import os
import sys
from core.system_context import get_system_context, CONFIG_PATH

LOGS_DIR = "logs"
LOG_FILE = os.path.join(LOGS_DIR, "praximous.log")

# Load system context only if the config file exists to avoid warnings in tools
if os.path.exists(CONFIG_PATH):
    # This import is safe here because we know the context can be loaded
    # We call the function to get the singleton instance.
    SYSTEM_NAME = get_system_context().display_name
else:
    # A specific name for tool-related logs or when uninitialized
    SYSTEM_NAME = "Praximous-Tool"
...
class ContextFilter(logging.Filter):
    ...
    def filter(self, record):
        # The SYSTEM_NAME is determined once at the module level.
        # This avoids repeated checks or dynamic imports.
