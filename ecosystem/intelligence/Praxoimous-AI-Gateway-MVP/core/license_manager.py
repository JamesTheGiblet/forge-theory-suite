...
def get_current_license_tier() -> LicenseTier:
    """
    Retrieves the current license tier. For development/testing, this can be
    overridden by setting the PRAXIMOUS_LICENSE_TIER environment variable.
    Otherwise, it uses the cryptographic license verification module.
    """
    # 1. Check for the test override environment variable first.
    # This is useful for development and CI/CD to test features without a full license key.
    test_tier_str = os.getenv("PRAXIMOUS_LICENSE_TIER")
    if test_tier_str:
        try:
            tier = LicenseTier(test_tier_str.lower())
            log.warning(f"Using test override license tier from PRAXIMOUS_LICENSE_TIER: {tier.name}")
            return tier
        except ValueError:
            log.warning(f"Invalid value '{test_tier_str}' for PRAXIMOUS_LICENSE_TIER. Defaulting to Community.")
            return LicenseTier.COMMUNITY

    # 2. If no override, proceed with the standard license key verification.
    ...
    if active_license and active_license.is_valid and not active_license.is_expired:
        ...
        log.info("No valid license found or license verification failed. Defaulting to Community tier.")
        return LicenseTier.COMMUNITY
