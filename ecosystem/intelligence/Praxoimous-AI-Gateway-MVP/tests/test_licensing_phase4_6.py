...
def test_get_active_license_info_file(monkeypatch, test_private_key, test_public_key, tmp_path):
    # 1. Create a temporary license file
    license_str = create_test_license_str(test_private_key, "File Customer", CoreLicenseTierEnum.ENTERPRISE, 60)
    license_file = tmp_path / "license.key"
    license_file.write_text(license_str)

    # 2. Ensure the environment variable is not set
    monkeypatch.delenv("PRAXIMOUS_LICENSE_KEY", raising=False)

    # 3. Patch the default path and the public key loading
    with patch("core.license.DEFAULT_LICENSE_PATH", str(license_file)), \
         patch("core.license.load_public_key", return_value=test_public_key):
        
        # Clear caches to ensure the function re-evaluates the license
        core.license._cached_license_info = None

        license_info = get_active_license_info()

    assert license_info is not None
    assert license_info.customer_name == "File Customer"
    assert license_info.tier == CoreLicenseTierEnum.ENTERPRISE
