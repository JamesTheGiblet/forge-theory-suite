  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // The data is nested under a 'data' key in the response model
        setData((result.data || []).map((item: ChartDataPoint) => ({ ...item, average_latency: parseFloat(item.average_latency.toFixed(0)) }))); // Round to integer for display
      } catch (err: any) {
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
        ...
        <Tooltip formatter={(value: string | number | (string | number)[]) => `${value} ms`} />
        <Legend />
