const path = require('path');

class SCPSpawner {
  constructor(engine) {
    this.engine = engine;
    this.spawnedAgents = new Map();
  }
  
  async spawnAll() {
    console.log('\n🔮 Spawning agents from SCP registry...\n');
    
    const agents = this.engine.getEnabledAgents();
    
    for (const agentConfig of agents) {
      await this.spawnAgent(agentConfig);
    }
    
    console.log(`\n✅ Spawned ${this.spawnedAgents.size} agents\n`);
    return this.spawnedAgents;
  }
  
  async spawnAgent(agentConfig) {
    try {
      // Dynamically load agent class
      const agentModule = require(`../agents/${agentConfig.name.toLowerCase().replace(/ /g, '_')}`);
      const AgentClass = agentModule[agentConfig.name];
      
      if (!AgentClass) {
        console.log(`[SPAWN] ${agentConfig.name}: No implementation, using BaseAgent`);
        const { BaseAgent } = require('../agents/base_agent');
        const agent = new BaseAgent(agentConfig, this.engine);
        await agent.start();
        this.spawnedAgents.set(agentConfig.scp_id, agent);
        this.engine.agents.set(agentConfig.scp_id, agent);
        return agent;
      }
      
      const agent = new AgentClass(agentConfig, this.engine);
      await agent.start();
      this.spawnedAgents.set(agentConfig.scp_id, agent);
      this.engine.agents.set(agentConfig.scp_id, agent);
      
      // Forward events
      agent.on('started', (data) => this.engine.emit('agent_started', data));
      agent.on('stopped', (data) => this.engine.emit('agent_stopped', data));
      agent.on('heartbeat', (data) => this.engine.emit('agent_heartbeat', data));
      agent.on('log', (log) => this.engine.emit('agent_log', log));
      
      return agent;
    } catch (err) {
      console.error(`[SPAWN] Failed to spawn ${agentConfig.name}: ${err.message}`);
      return null;
    }
  }
  
  getAgent(scpId) {
    return this.spawnedAgents.get(scpId);
  }
  
  async stopAll() {
    console.log('\n🛑 Stopping all agents...');
    for (const [scpId, agent] of this.spawnedAgents) {
      await agent.stop();
    }
    this.spawnedAgents.clear();
    console.log('✅ All agents stopped\n');
  }
}

module.exports = { SCPSpawner };
