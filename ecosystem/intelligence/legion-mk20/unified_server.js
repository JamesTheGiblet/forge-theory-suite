...
app.post('/api/strategies/rate', (req, res) => {
  ...
  if (!strategyId || rating < 1 || rating > 5) return res.status(400).json({ error: 'Invalid input' });
  const indexPath = './data/strategy_index.json';
  if (!fs.existsSync(indexPath)) return res.status(404).json({ error: 'No strategies' });
  let index = {};
  try { index = JSON.parse(fs.readFileSync(indexPath, 'utf8')); } catch(e) { return res.status(500).json({ error: e.message }); }
...
app.get('/api/strategies/gallery', (req, res) => {
  ...
  const strategies = Object.entries(index).map(([id, data]) => ({
    id, name: data.name, class: data.class, fitness: data.fitness || 0,
    userRating: data.userRating || 0, lineage: data.lineage || {}, fingerprint: data.fingerprint
  }));
...
app.post('/api/strategies/breed', (req, res) => {
  ...
  if (!parentA || !parentB) return res.status(404).json({ error: 'Parent not found' });
  const child = {
    ...
    lineage: { parents: [parentA_id, parentB_id], generation: Math.max(parentA.lineage?.generation||0, parentB.lineage?.generation||0) + 1, timestamp: Date.now() },
...
app.get('/api/strategies/patterns', (req, res) => {
  const indexPath = './data/strategy_index.json';
  if (!fs.existsSync(indexPath)) return res.json({ patterns: {}, sampleSize: 0 });
  let index = {};
  try { index = JSON.parse(fs.readFileSync(indexPath, 'utf8')); } catch(e) { return res.status(500).json({ error: e.message }); }
  const strategies = Object.values(index).sort((a,b) => (b.fitness||0) - (a.fitness||0)).slice(0, 50);
  const patternCounts = {};
  for (const s of strategies) patternCounts[s.class] = (patternCounts[s.class] || 0) + 1;
  const patterns = Object.fromEntries(Object.entries(patternCounts).map(([k,v]) => [k, { count: v, ratio: v / strategies.length }]));
...
app.listen(PORT, () => {
  ...
  console.log(`   Forge integration endpoints active`);
});
