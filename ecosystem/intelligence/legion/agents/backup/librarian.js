...
function indexStrategies() {
  ...
  for (const [id, data] of Object.entries(index)) {
    if (fingerprints[data.fingerprint]) {
      // console.log(`[LIBRARIAN] ⚠️ DUPLICATE: ${id} matches ${fingerprints[data.fingerprint]}`);
      duplicateCount++;
    } else {
  ...
  // console.log(`[LIBRARIAN] Indexed ${Object.keys(index).length} strategies (${duplicateCount} duplicates)`);
  return index;
}
...
function startLibrarian() {
  ...
  registerHandler('VALIDATION_COMPLETE', (msg) => {
    // console.log(`[LIBRARIAN] New strategy: ${msg.payload.strategyId}`);
    setTimeout(() => {
      indexStrategies();
      if (msg.payload.objectClass === 'Keter') {
        // console.log(`[LIBRARIAN] Voting YES for Keter strategy: ${msg.payload.strategyId}`);
