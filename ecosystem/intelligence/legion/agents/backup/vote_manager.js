...
function addVoteToLedger(strategyId, votes, result) {
  ...
  // console.log(`[VOTE] Recorded ${result} for ${strategyId}`);
  return newEntry;
}
...
function requestKeterVote(strategyId, strategy, requester) {
  ...
  // console.log(`[VOTE] ${requester} voted for ${strategyId} (${voteSet.size}/${KETER_VOTES_REQUIRED})`);
...
function startVoteManager() {
  ...
  if (isValid) {
    // console.log(`[VOTE] Ledger valid – ${ledger.chain.length} entries`);
  } else {
    // console.log(`[VOTE] Ledger reset – starting fresh`);
  }
  ...
  // console.log('[VOTE] Manager ready – Keter requires 3-agent consensus');
}
