...
function loadState() {
  if (fs.existsSync(STATE_FILE)) {
    try {
      ...
      // console.log('[WHALE] Loaded state');
    } catch (e) { // console.log('[WHALE] New state'); }
  }
}
...
async function checkWhaleActivity() {
  // console.log('[WHALE] Checking on-chain activity...');
  ...
  // console.log(`[WHALE] Total whale value: $${(summary.totalValue / 1000000).toFixed(1)}M, Sentiment: ${summary.sentiment}`);
}

async function startWhaleAlert() {
  loadState();
  // console.log('[WHALE] Ready – checking every 30 minutes');
