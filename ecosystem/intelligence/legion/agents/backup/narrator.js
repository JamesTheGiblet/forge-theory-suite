...
class Narrator {
    ...
    exec(`termux-tts-speak "${text.replace(/[^a-zA-Z0-9 .,!?]/g, '')}"`, (err) => {
      if (err) {
        ...
        exec(`espeak "${text.replace(/[^a-zA-Z0-9 .,!?]/g, '')}"`, (err2) => {
          if (err2) // console.log('[NARRATOR] Speech not available');
        });
      }
    });
  }
  ...
  async narrate() {
    // console.log('[NARRATOR] Gathering intelligence...');
    ...
    // console.log('[NARRATOR] Narrative broadcasted.');
    return narrative;
  }

  start(intervalMinutes = 60) {
    // console.log(`[NARRATOR] Voice active – narrating every ${intervalMinutes} minutes`);
    // Narrate immediately
    this.narrate();
    // Then schedule
