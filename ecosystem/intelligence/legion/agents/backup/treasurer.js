...
function recordTradeResult(strategyId, isWin) {
  ...
  if (isWin) {
    state.wins++;
    state.losses = 0;
    // console.log(`[TREASURER] ${strategyId}: win streak = ${state.wins}`);
    ...
    if (state.wins >= WIN_STREAK_TO_INCREASE && state.positionSize < MAX_POSITION_SIZE) {
      ...
      // console.log(`[TREASURER] ${strategyId}: position size increased to $${state.positionSize}`);
      sendMessage('diplomat', 'POSITION_CHANGE', { strategyId, newSize: state.positionSize, reason: `${state.wins} consecutive wins` });
    }
  } else {
    state.losses++;
    state.wins = 0;
    // console.log(`[TREASURER] ${strategyId}: loss streak = ${state.losses}`);
    ...
    if (state.losses >= LOSS_STREAK_TO_PAUSE) {
      ...
      // console.log(`[TREASURER] ${strategyId}: PAUSED for ${PAUSE_DURATION_HOURS}h`);
      ...
      // console.log(`[TREASURER] ${strategyId}: position size reduced to $${state.positionSize}`);
    }
  }
...
function startTreasurer() {
  loadState();
  // console.log(`[TREASURER] Ready – tracking ${strategyStates.size} strategies`);
  ...
  registerHandler('PROMOTE_STRATEGY', (msg) => {
    // console.log(`[TREASURER] Strategy ${msg.payload.strategyId} promoted! PnL: ${(msg.payload.pnl*100).toFixed(1)}%`);
  });
  ...
  setInterval(() => {
    ...
    // console.log(`[TREASURER] Status: ${active} active, ${paused} paused`);
  }, 60 * 60 * 1000);
}
