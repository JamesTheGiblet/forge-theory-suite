...
function discoverTemplates() {
  // console.log(`[SCOUT] Scanning for new strategy templates`);
  
  for (const template of templates) {
    const templatePath = path.join(TEMPLATE_DIR, `${template.name}.template.json`);
    if (!fs.existsSync(templatePath)) {
      fs.writeFileSync(templatePath, JSON.stringify(template, null, 2));
      // console.log(`[SCOUT] Created template: ${template.name}`);
      sendMessage('forge_lord', 'NEW_TEMPLATE', { template: template.name, path: templatePath });
    }
  }
  
  // console.log(`[SCOUT] Discovery complete – ${templates.length} templates available`);
  process.exit(0);
}
