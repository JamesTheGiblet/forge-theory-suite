...
function loadLineageHistory() {
  if (fs.existsSync(LINEAGE_HISTORY)) {
    try {
      ...
      // console.log(`[CHAMELEON] Loaded ${lineageHistory.size} lineages`);
    } catch (err) {
...
function recordMutation(parentId, mutation, backtestResult) {
  ...
  // console.log(`[CHAMELEON] Lineage ${parentId}: ${assessment.status} (${assessment.reason})`);
...
function startChameleon() {
  loadLineageHistory();
  // console.log(`[CHAMELEON] Ready – tracking ${lineageHistory.size} lineages`);
  ...
  setInterval(() => {
    ...
    // console.log(`[CHAMELEON] Status: ${stats.total} lineages | Thrashing: ${stats.thrashing} | Evolving: ${stats.evolving} | Uncertain: ${stats.uncertain}`);
  }, 60 * 60 * 1000);
}
